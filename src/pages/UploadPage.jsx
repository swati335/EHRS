...
const UploadPage = () => {
 ...
 const fetchUserRole = async () => {
  ...
  try {
   const registryContract = contracts.RegistryContract;
    // console.log("SmartContractsContext:", smartContractsContext);
    if (!registryContract) {
    ...
    const userData = await registryContract.checkUser(signer.address);


    const roleId = Number(userData[1]); // Convert BigInt to Number
    let roleName = "";
  ...
  const handleUpload = async () => {
    ...
    try {
      if (role === "CareProvider") {
        // Doctor is requesting to upload on behalf of a user
        await permissionsContract.requestDoctorUpload(userId, JSON.stringify(userData));
        showNotification("Upload request sent to user for approval.", "success");
      } else {
        // Normal user uploads data directly
      ...
      {loading &&  role !== "" ? (
        ...
        <div className="p-4 w-1/3 flex flex-col items-center">
         ...
         rows="3" />
          ...
          <Input type="date" 
          placeholder="Enter Date of Admission" value={dateOfAdmission} 
          onChange={(e) => setDateOfAdmission(e.target.value)} className="mb-2  text-white bg-gray-800 rounded-md border border-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-200" />

          <Button onPress={handleUpload} className="mt-3" color="primary">
            {role === "CareProvider" ? "Request Upload" : "Upload Data"}
          </Button>
        </div>
      )}
    </Container>
